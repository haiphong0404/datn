...
class UpdateVoucherRequest extends FormRequest
{
    ...
    {
        $rules = [
            'code' => 'required|string|unique:vouchers,code,' . $this->route('voucher')->id, // Chỉ kiểm tra unique khi không phải là mã của voucher hiện tại
        ...
        // Chỉ kiểm tra các quy tắc đặc biệt khi loại voucher được chọn
        if ($this->type) {
            switch ($this->type) {
                case 'percentage':
